...
var Pas =
{
    ...
    canPlace: function(row) {
        ...
        if (row >= 9 && row < 13 && this.currentCard < 52) {// Êëàäåì â êó÷ó
        ...
        if (row >= 0 && row < 9) {
            ...
            // Â ïóñòîé ðÿä êëàä¸ì âñ¸ ïîäðÿä, êðîìå äæîêåðà.	
            if (this.layout[row].length == 0)
                return this.currentCard < 52 ? true : false;

            // Äæîêåðà ìîæíî ïîëîæèòü êóäà õî÷åøü, òîëüêî íå â ïóñòîé ðÿä
            if (this.currentCard >= 52)
            ...
            if (dRange == 1) {
                ...
                if (dRange == 2) {
                if ((underCard % 4 < 2 ? 0 : 1) == cardSuit) {// Åñëè åñòü õîòÿ áû 1 äæîêåð
                ...
                if (dRange == 3) {
                if ((underCard % 4 < 2 ? 0 : 1) != cardSuit) {// Åñëè åñòü 2 äæîêåðà
                    if (this.layout[13].length != 0 && this.layout[14].length != 0) {
                        this.nJokers = 2;
                        return true;
                    }
    ...
    updateCursor: function(row) {
        if (row < 9 && this.layout[row].length > 1) {// Óñòàíîâèòü êóðñîð
	...
	if( number == 1)		
		...
		cards = new Array (9,38,37,17,50,30,51,41,42,4,1,49,21,46,2,47,44,29,19,27,35,6,0,43,14,15,11,18,22,8,28,10,26,16,39,13,20,12,7,33,3,24,48,45,36,5,40,52,32,25,53,34,23,31);
        for(row = 0; row < 6; row++)
        {
            for(col = 0; col < 9; col++)
            {
